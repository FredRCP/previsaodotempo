document.addEventListener('DOMContentLoaded', () => {
  ...
  async function getWeather(city) {
    const url = `https://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=${city}&days=6&lang=pt`; // Alterando para forecast.json com 5 dias de previsão
...
document.getElementById('city-name').innerHTML = `${data.location.name} - ${today}`;




          // Exibir o nome da cidade no HTML
         ...
         // Exibir a previsão dos próximos 5 dias (excluindo hoje, se necessário)
...
// Se o primeiro dia for hoje, remova-o
const todayDate = new Date().toISOString().split('T')[0];
if (forecastDays[0].date === todayDate) {
    forecastDays.shift();  // Remove o primeiro dia
}

// Exibir os próximos 5 dias corretamente
